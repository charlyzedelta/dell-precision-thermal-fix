...
cleanup() {
  ...
  say "restored: PL1=$(( $(cat $R/constraint_0_power_limit_uw)/1000000 ))W"
}
...
set_pl() {  # set_pl <PL1_W>  -- PL2 = PL1+15, tau = 8s, single variable
  local p1=$1
  echo $(( p1 * 1000000 ))         > $R/constraint_0_power_limit_uw
  echo 8000000                     > $R/constraint_0_time_window_us
  echo $(( (p1 + 15) * 1000000 ))  > $R/constraint_1_power_limit_uw
  # Read it back. Setting the independent variable and assuming it held is
  # exactly how the first attempt produced ten runs of identical data.
  local got=$(( $(cat $R/constraint_0_power_limit_uw) / 1000000 ))
  [ "$got" = "$p1" ] || { say "FATAL: asked for ${p1}W, sysfs reads ${got}W -- something is re-asserting the cap"; exit 1; }
}
pl_now() { echo $(( $(cat $R/constraint_0_power_limit_uw) / 1000000 )); }
